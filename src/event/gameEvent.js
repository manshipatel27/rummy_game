...
//   ======================== C ??? joinRoom  ========================>>>
  ...
  socket.on("discardCard", ({ roomId, playerId, card }) => {
    try {
      ...
      if (player.hand.length === 0) {
        console.log("Player has no cards left after discard! Declaring winner:", playerId);
        player.score = 0;
  ...
  socket.on("layDownMelds", ({ roomId, playerId, melds }) => {
    try {
      ...
      // UPDATED GAMEOVER CHECK IN LAYDOWNMELDS
      if (player.hand.length === 0) {
        console.log("Player has no cards left after melds! Declaring winner:", playerId);
        player.score = 0;
